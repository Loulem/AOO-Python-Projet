...
def test_add_reservation_error():
    ...
    controller.add_room("Room1", "Conference")
    mael_uuid = controller.get_client_uuid("Mael")
    controller.add_reservation("Room1", TimeInterval(date(2026, 9, 6), time(12, 30), timedelta(hours=1, minutes=10)), "mael@uha.fr")
    with pytest.raises(Reservation_app_error):
        # test if the reservation is in the past
        controller.add_reservation("Room1", TimeInterval(date(2020, 10, 1), time(12, 30), timedelta(hours=1, minutes=10)), "mael@uha.fr")
    with pytest.raises(Reservation_app_error):
        # test if the reservation is not available
        controller.add_reservation("Room1", TimeInterval(date(2026, 9, 6), time(12, 30), timedelta(hours=1, minutes=10)), "mael@uha.fr")
    with pytest.raises(Reservation_app_error):
        # test if the room does not exist
        controller.add_reservation("Room2", TimeInterval(date(2026, 9, 6), time(12, 30), timedelta(hours=1, minutes=10)), "mael@uha.fr")
    with pytest.raises(Reservation_app_error):
